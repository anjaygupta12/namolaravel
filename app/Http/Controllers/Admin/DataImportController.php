...
class DataImportController extends Controller
{
    ...
    public function importDataAuto()
    {
        ...
        foreach($impUrl as $val){
        $this->importData($val);

        }
        // $impUrl = 'https://public.fyers.in/sym_details/MCX_COM.csv';
        // dd($this->importData($impUrl));
    }


    public function importData($impUrl)
    {
        ...
        try {
            ...
            $response = Http::timeout(300)->get($impUrl);
    ...
    public function importData__($impUrl)
    {
        set_time_limit(36000);
        ...
        try {
            ...
            // Fetch data from URL
            Log::info('Fetching data from URL: ' . $impUrl);
            try {
                $response = Http::timeout(300)->get($impUrl);

                if (!$response->successful()) {
                    Log::error('Failed to fetch data from URL: ' . $impUrl . ', Status: ' . $response->status());
    ...
    public function importDataMunu(Request $request)
    {
        set_time_limit(36000);
        ini_set('memory_limit', '1G');
    ...
    protected function __parseCsvData($csvData, $exchangeName, $limit = 1000)
    {
        $options = [];
        $lines = explode("\n", $csvData);
        $count = 0;
        ...
        foreach ($lines as $lineIndex => $line) {
            ...
            // if($exchangeInstrument='MCX:COPPER26APR1330CE'){

                // dd($expiryDate->format('Y-m-d'));
            // }
...
protected function parseCsvData($csvData, $exchangeName, $limit = 1000)
{
    ...
    foreach ($lines as $lineIndex => $line) {
        ...
        if (!isset($fields[0]) || empty($fields[0])) continue;

        if ($count >= $limit) break;

        $id = $fields[0] ?? 0;
        $description = $fields[1] ?? '';
        $exchangeCode = $fields[2] ?? 0;
        $segment = $fields[3] ?? 0;
        $tickSize = $fields[4] ?? 0.05;
        $tradingSession = $fields[6] ?? '';
        $dataDateStr = $fields[7] ?? '';
        $expiryTimestamp = $fields[8] ?? 0;
        $exchangeInstrument = $fields[9] ?? '';
        $lotSize = $fields[10] ?? 1;
        $instrumentType = $fields[11] ?? 0;
        $fyToken = $fields[12] ?? 0;
        $underlyingFromCsv = $fields[13] ?? '';
        $multiplier = $fields[14] ?? 1;
        $strikePrice = $fields[15] ?? 0;
        $optionTypeRaw = $fields[16] ?? '';
        $fyTokenUnderlying = $fields[20] ?? 0;

        // Extract underlying
        $underlying = 'Unknown';
        if (!empty($description)) {
            $parts = explode(' ', $description);
            $underlying = $parts[0] ?? 'Unknown';
        }

        // ✅ Correct Instrument Detection
        if ($optionTypeRaw == 'CE') {
            $instrument = 'CE';
        } elseif ($optionTypeRaw == 'PE') {
            $instrument = 'PE';
        } elseif ($optionTypeRaw == 'XX' || strpos($exchangeInstrument, 'FUT') !== false) {
            $instrument = 'FUT';
        } else {
            $instrument = 'FUT';
        }
        ...
        if ($instrument == 'CE' || $instrument == 'PE') {
            preg_match('/\d+(\.\d+)?/', $description, $matches);
            if (!empty($matches[0])) {
                $strikePrice = $matches[0];
            }
        ...
        try {
            ...
            elseif (preg_match('/\d{1,2}\s[A-Za-z]{3}\s\d{2}/', $description, $match)) {
                $expiryDate = Carbon::createFromFormat('d M y', $match[0]);
            } 
            elseif (!empty($dataDateStr)) {
                $expiryDate = Carbon::parse($dataDateStr);
            } 
            else {
                $expiryDate = Carbon::now();
        ...
        if (is_numeric($fyToken) && strlen($fyToken) <= 9) {
            ...
            $safeId = abs(crc32($fyToken . $description)) % 1000000000;
        }

        $options[] = [

            'Id' => $id,
            'Symbol' => $exchangeInstrument,
            'Description' => $description,
            'Exchange' => intval($exchangeCode),
            'Segment' => intval($segment),
            'TickSize' => floatval($tickSize),
            'TradingSession' => $tradingSession,
            'ExpiryDate' => $expiryDate->format('Y-m-d'),
            'InstrumentToken' => intval($expiryTimestamp),
            'ExchangeInstrument' => $exchangeInstrument,
            'LotSize' => intval($lotSize),
            'InstrumentType' => intval($instrumentType),
            'FyToken' => intval($fyToken),
            'Underlying' => $underlyingFromCsv ?: $underlying,
            'Multiplier' => intval($multiplier),
            'StrikePrice' => floatval($strikePrice),
            'OptionType' => $instrument,
            'FyTokenUnderlying' => intval($fyTokenUnderlying),
            'ExName' => $exchangeName,
            'SymbolShortName' => $underlying,
            'DataDate' => !empty($dataDateStr) 
                ? Carbon::parse($dataDateStr)->format('Y-m-d') 
                : now()->format('Y-m-d'),
    ...
    protected function saveToDatabase__($options)
    {
        $importedCount = 0;
        ...
        foreach ($options as $checkOption) {
            if (
                $checkOption['Id'] == '1120250805438425' ||
    ...
    protected function saveToDatabase($options)
{
    ...
    foreach ($options as $option) {
        ...
        try {
            ...
            $option['Id'] = (int) ($option['Id'] ?? rand(100000, 999999));
            $option['Isactive'] = true;
            $option['LastUpdateDate'] = now()->format('Y-m-d H:i:s');

            $fyToken = $option['FyToken'] ?? 0;
            $fyToken = (is_numeric($fyToken) && $fyToken > $maxIntValue) 
                        ? $maxIntValue 
                        : (int) $fyToken;

            $instrumentToken = $option['InstrumentToken'] ?? 0;
            $instrumentToken = (is_numeric($instrumentToken) && $instrumentToken > $maxIntValue) 
                        ? $maxIntValue 
                        : (int) $instrumentToken;

            $fyTokenUnderlying = $option['FyTokenUnderlying'] ?? 0;
            $fyTokenUnderlying = (is_numeric($fyTokenUnderlying) && $fyTokenUnderlying > $maxIntValue) 
                        ? $maxIntValue 
                        : (int) $fyTokenUnderlying;

            $multiplier = $option['Multiplier'] ?? 0;

            if (is_numeric($multiplier)) {
                $multiplier = max(min($multiplier, 32767), -32768);
            } else {
                $multiplier = 0;
            }
        //     if($option['Symbol']=='MCX:COPPER26APRFUT'){
        //     dd($option);
        ...
        // }
            ...
            $result = DB::table('forexoptions')->updateOrInsert(
                ...
                [
                    'Id' => $option['Id'],
                    'Symbol' => substr($option['Symbol'] ?? '', 0, 50),
                    'Description' => substr($option['Description'] ?? '', 0, 255),
                    'Exchange' => (int) ($option['Exchange'] ?? 0),
                    'Segment' => (int) ($option['Segment'] ?? 0),
                    'TickSize' => (float) ($option['TickSize'] ?? 0),
                    'TradingSession' => substr($option['TradingSession'] ?? '', 50),
                    'ExpiryDate' => $option['ExpiryDate'] ?? now()->format('Y-m-d'),
                    'InstrumentToken' => $instrumentToken,
                    'ExchangeInstrument' => substr($option['ExchangeInstrument'] ?? '', 0, 50),
                    'LotSize' => (int) ($option['LotSize'] ?? 0),
