...
{
    // public function ___updatePendingOrder($id)
    // {

    //     $homeCtrl = new HomeController();

    //     // Node orderActivationCache sends trade_id in the URL (same as local DB activation).
    //     $order = MarketBidMaster::where('trade_id', $id)->where('Isactive', 0)->first();
    ...
    //             $marginAvilabe = $homeCtrl->getMarginCheckAvilable($user, $SymbolShortName, $lotSize, $order->TransactionMode, $order->Lots, $order->BuyPrice);

    //             $checked =  $this->checkQtyExist($order, $existTrade, $marginAvilabe, $order->Lots);

    //             if ($checked['error'] == false) {
    //                 DB::table('marketbidmaster')->where('trade_id', $id)
    //                     ->update([
    //                         'Isactive' => 2,
    //                         'deleted_at' => null
    //                     ]);
    //                 return response()->json(['message' => $order->Mode . 'Already exists and then closed successfully']);
    //             } else {
    //                 return response()->json(['message' => $checked['message']]);
    //             }
    //         } else {
    //             $updated = DB::table('marketbidmaster')->where('trade_id', $id)
    //                 ->update([
    //                     'Isactive' => 1,
    //                     'deleted_at' => null
    //                 ]);
    ...
    //                     $existTrade->Lots = $existTrade->Lots - $order->Lots;
    //                     $existTrade->save();

    //                     $newData1 = $existTrade->toArray();
    //                     unset($newData1['Pk_id']);
    //                     $newData1['Lots'] = $order->Lots;
    //                     $newData1['trade_id'] = rand(10000000, 99999999);
    //                     $newData1['holding_margin_req'] = $holdingmargin * $order->Lots;
    //                     $newData1['used_margin_req'] = $usedmargin * $order->Lots;
