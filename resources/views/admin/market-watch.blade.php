    ...
    <div class="container">
        ...
        <div class="search-container mb-4">
            <div class="input-group input-group-lg">
                ...
                <input type="text" id="global-search-box" class="form-control"
                    placeholder="Search symbols, names, or any market data...">
...
@section('scripts')
    <script>
        $(document).ready(function() {
            ...
            let selfBlocked = @json($selfBloacked);
            let parentBloacked = new Set(@json($parentBloacked));
            ...
            function connect() {
                try {
                    ...
                    //  const wsUrl = 'wss://namotraders.in:5005/ws?isBroker=true&userId=2';    
                                ...
                                data.forEach(function(item) {
                                    if (item && item.Data) {
                                        ...
                                        if (item.Data.expiryDate) {
                                            ...
                                            if (!isNaN(expiryDate)) {
                                                const day = expiryDate.getDate().toString().padStart(2,
                                                    '0');
            ...
            function filterMarketData(exchange) {
                return Object.entries(marketData).filter(function(entry) {
                    ...
                    // Hide row if symbol exists in parentBloacked
                    if (parentBloacked.has(symbol)) {
                        return false;
                    }
                    ...
                    if (exchange === 'nse') {
                        ...
                        isCorrectExchange = symbol.startsWith('FOREX:') ||
                    ...
                    return (
                        ...
                        (data.symbolShortName &&
                            data.symbolShortName.toLowerCase().includes(term)) ||
                        (data.expiryDate &&
                            data.expiryDate.toLowerCase().includes(term)) ||
                        (data.ltp &&
                            data.ltp.toString().includes(term)) ||
                        (data.bid_price &&
                            data.bid_price.toString().includes(term)) ||
                        (data.ask_price &&
                            data.ask_price.toString().includes(term)) ||
                        (data.ch &&
                            data.ch.toString().includes(term)) ||
                        (data.chp &&
                            data.chp.toString().includes(term)) ||
                        (data.vol_traded_today &&
                            data.vol_traded_today.toString().includes(term)) ||
                        (data.open_price &&
                            data.open_price.toString().includes(term))
                    );
                });
            }
            ...
            function updateTable(type, colspan) {
                ...
                const tbody = document.getElementById(
                    `${type}-data-body`); // make sure table body id matches (e.g. id="nse-data-body")
            ...
            function createTableRow(symbol, data) {
                ...
                // Create table cells
                const cells = [
                    createCell(symbol.split(':')[1] || symbol),
                ...
                const isSelfBlocked = selfBlocked.includes(symbol);
                const isBanned = isSelfBlocked;

                const href = 'banned-status/?status=' + data.banned + '&sybmol=' + symbol;

                bannedButton.href = href;
                bannedButton.textContent = isBanned ? 'REMOVE BAN' : 'ADD TO BAN';
                ...
                bannedButton.style.backgroundColor = isBanned ? '#007bff' : '#dc3545';
