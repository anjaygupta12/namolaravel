...
<body>
    <h2>
        {{ $user->user_id . ' : (' . $user->Username . ') ' . $user->FullName }}
    </h2>
    <h2>Billing for:  ({{ $from }} - {{ $to }})</h2>
    @php
        $allTotal_pa = 0;
        $allTotalBrokerage = 0;
        $netAmount = 0;
        ...
        <tbody>
            ...
            @forelse($mxcPendingTrade as $trade)
                @php
                    ...
                    $buyTime = \Carbon\Carbon::parse($trade->created_at)->format('Y-m-d H:i:s A');
                    $sellTime = \Carbon\Carbon::parse($trade->updated_at)->format('Y-m-d H:i:s');
                ...
                <tr>
                    ...
                    <td>{{ $lots }} / {{ $trade->is_qty == 1 ? $trade->quentity : $lotSize }}</td>
    ...
    <table class="pending-table">
        <thead>
         ...
         <tbody>
            ...
            @forelse($carryForwordTrade as $trade)
                
                <tr>
                    <td>{{ explode(':', $trade->Symbol)[1] ?? $trade->Symbol }}</td>
                    <td>{{ $trade->Mode }}</td>
                    <td>{{ $trade->TransactionMode }}</td>
                    <td>{{ $trade->Lots }} / {{ $trade->is_qty == 1 ? $trade->quentity : $lotSize }}</td>

                    <td>{{ number_format($trade->BuyPrice, 2) }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="11" class="text-center">No records found</td>
